...
class Solution(object):
    def possibleBipartition(self, N, dislikes):
        dislikes = [sorted(x) for x in dislikes]
        m = defaultdict(list)
        for a, b in dislikes:
            m[a].append(b)
            m[b].append(a)
        ...
        for i in xrange(1, N+1):
            ...
            if has_odd_cycle(i, m, visited):
                return False

        return True


def has_odd_cycle(root, m, visited):
    visited[root] = 0
    q = [root]
    while q:
        n = q.pop()
        ...
        for node in nodes:
            if node in visited:
                visited_parity = visited[node]
                if parity != visited_parity:  # odd cycle
                    return True
                else:
                    continue
