...
class Solution(object):
    ...
    def rob(self, root):
        return max(
            _rob(root, selected=False),
            _rob(root, selected=True),
        )


def _rob(node, selected=False):
    if not node:
        return 0

    if selected:
        return node.val + _rob(node.left, selected=False) + _rob(node.right, selected=False)

    left_v = max(
        _rob(node.left, selected=False),
        _rob(node.left, selected=True),
    )
    right_v = max(
        _rob(node.right, selected=False),
        _rob(node.right, selected=True),
    )
